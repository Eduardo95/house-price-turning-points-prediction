...
获得这一时刻的状态（拐点或者非拐点）
'''
hpi_data = build_spain_HPI_index_dataset.index_temp
google_data = build_spain_HPI_index_dataset.google_temp
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = "1"
'''
这里计算西班牙指数的变动方向，下一时刻比这一时刻值更高，则记为1，否则记为0
'''
direction = list()
for i in range(len(hpi_data) - 1):
    if hpi_data[i + 1] > hpi_data[i]:
        direction.append(1)
    elif hpi_data[i + 1] < hpi_data[i]:
        direction.append(0)
direction.append(-1)
up_features = list()
down_features = list()
...
for i in range(148, 178, 1):
    train_features = list()
    train_labels = list()
    test_features = list()
    test_labels = list()
    test_features.append(hpi_data[i - 11:i + 1])
    test_labels.append(direction[i])
    for j in range(11, i, 1):
        train_features.append(hpi_data[j - 11:j + 1])
